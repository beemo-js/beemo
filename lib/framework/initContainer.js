...
import { AnnotationsServiceName, ConfigServiceName, ConversionServiceName, DiServiceName, EventsServiceName, HttpServiceName, LoggingServiceName, MetadataServiceName, SerializationServiceName, ThreadsServiceName, TypesServiceName, ValidationServiceName } from './services';
...
export function initContainer() {
    ...
    container.set(DiServiceName.ReflectionServiceManager, () => new ReflectionServiceManager(container.get(TypesServiceName.ReflectionClassTypesStore), container.get(AnnotationsServiceName.ClassAnnotationsStore), container.get(DiServiceName.Container), container.get(ConfigServiceName.ConfigurationStore)));
    // Events
    container.set(EventsServiceName.EventBus, () => new EventBus());
    // HTTP
    container.set(HttpServiceName.CallAnnotationsHandler, () => new CallAnnotationsHandler(container.get(AnnotationsServiceName.ClassAnnotationsStore), container.get(SerializationServiceName.Normalizer)));
    container.set(HttpServiceName.CallHttpClient, () => new CallHttpClient(container.get(HttpServiceName.HttpClient), container.get(SerializationServiceName.Encoder), container.get(SerializationServiceName.Normalizer)));
    container.set(HttpServiceName.RequestsQueue, () => new BatchRequestsQueue(container.get(HttpServiceName.HttpClient), false));
    ...
    container.set(LoggingServiceName.LogsOrchestrationWorker, () => new LogsOrchestrationWorker(container.get(LoggingServiceName.LoggingBag), container.get(LoggingServiceName.LogsOrchestrator), 5));
    container.set(LoggingServiceName.LogsOrchestrator, () => new HttpLogsOrchestrator(container.get(HttpServiceName.RequestsQueue), log => new RequestBuilder().url('/log').body(log).build()));
    // Metadata
    container.set(MetadataServiceName.ClassMetadataStore, () => new ClassMetadataStore());
    // Serialization
    container.set(SerializationServiceName.Serializer, () => new ComposableSerializer(container.get(SerializationServiceName.Normalizer), container.get(SerializationServiceName.Encoder)));
    ...
    // Threads
    container.set(ThreadsServiceName.BackgroundLoop, () => new BackgroundLoop(container.get(ThreadsServiceName.BackgroundTaskManager), 10));
    // Types
